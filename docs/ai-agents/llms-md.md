# CLAUDE.md / LLMS.md — Документ для AI-агентов

## Назначение

Генератор создаёт документ-инструкцию для AI-кодинг агентов (Claude Code, Codex, Cursor и др.) в корень проекта. После прочтения агент понимает архитектуру, правила и может работать без дополнительных вопросов.
...
Генератор автоматически выбирает имя файла при первой генерации:

| Состояние target-директории | Генерируемый файл | Причина |
|---|---|---|
| Нет `CLAUDE.md` | `CLAUDE.md` | Предпочтительный вариант — Claude Code автоматически читает этот файл |
| Есть `CLAUDE.md` пользователя | `LLMS.md` | Fallback — чтобы не перезаписать существующий файл пользователя |

При **регенерации** (`make regenerate`) генератор сохраняет ранее выбранное имя файла.

### Если сгенерировался LLMS.md

Если у вас уже был свой `CLAUDE.md` и генератор создал `LLMS.md`, добавьте в ваш `CLAUDE.md` указание агенту:

```markdown
Read and follow the instructions in LLMS.md — it contains the generated project architecture guide.
```

Это гарантирует, что AI-агент прочитает сгенерированные инструкции по архитектуре проекта.

## Поведение при регенерации

Файл генерируется **один раз**. При повторном запуске `make regenerate` файл **не перезаписывается** (аналогично `README.md` и `LICENSE.txt`). Это позволяет добавлять в него проектно-специфичные правила.
