...
func splitDisclaimer(fileContent string, disclaimerStartWithComment string, disclaimerFinishtWithComment string) (string, string, error) {
	...
	// var disclaimerFindStart string
	disclaimerFindStart := fileContent[:disclamerStartIndex+len(disclaimerStart)+1]
	if disclaimerFindStart == "" || len(disclaimerFindStart) == 0 {
		return fileContent, "", errors.New("The starting disclaimer was not found.")
	}

	// disclaimerFindFinish := fileContent[disclamerFinishIndex+len(DisclaimerFinish)+1]

	codeByGenerator := fileContent[disclamerStartIndex+len(disclaimerStart)+1 : disclamerFinishIndex]
	if codeByGenerator == "" || len(codeByGenerator) == 0 {
		return fileContent, "", errors.New("The code generated by the utility was not found.")
	}
